...
def generate_last_week_markdown():
    ...
    md_lines = []
    md_lines.append("# Math Engine Only - Last Week Backtest")
    md_lines.append("\nThis backtest executes every single trade triggered by the PIM Math Engine from August 10 to August 14, bypassing the Debate Engine (LLM) completely.")
    ...
    for t_date in target_dates:
        ...
        md_lines.append(f"\n## Date: {t_date}")
        md_lines.append("| Time (IST) | Symbol | Entry Price | Max Excursion (%) | EOD Exit Price | EOD P&L (%) |")
        md_lines.append("|---|---|---|---|---|---|")
        ...
        for row in unique_times.iter_rows():
            ...
            for sym in valid_symbols:
                ...
                md_lines.append(f"| {ist_str} | **{sym}** | {entry_price:.2f} | +{max_exc_pct:.2f}% | {eod_close:.2f} | {eod_pnl_pct:.2f}% |")
                daily_pnl += eod_pnl_pct
                daily_trades += 1
                total_pnl += eod_pnl_pct
                total_trades += 1
                
        md_lines.append(f"\n**Daily P&L:** {daily_pnl:.2f}% (over {daily_trades} trades)")

    md_lines.append("\n## Final Summary")
    md_lines.append(f"- **Total Trades:** {total_trades}")
    md_lines.append(f"- **Net Cumulative P&L:** {total_pnl:.2f}%")
    md_lines.append(f"- **Average Trade Return:** {(total_pnl/total_trades):.2f}%" if total_trades > 0 else "")
    
    with open("/Users/poonamsalke/.gemini/antigravity/brain/310c2a32-4d78-4de7-9a47-64d8d330ff45/last_week_math_backtest.md", "w") as f:
        f.write("\n".join(md_lines))
    print("Generated artifact.")

if __name__ == "__main__":
    generate_last_week_markdown()
