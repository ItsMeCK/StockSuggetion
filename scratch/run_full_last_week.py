    ...
    for t_date in target_dates:
        ...
        for row in unique_times.iter_rows():
            ...
            if len(valid_symbols) > 0:
                ...
                print(f"🕒 RUN: {ts} | 🧠 Passing {len(debate_candidates)} to Orchestrator: {[c['symbol'] for c in debate_candidates]}")
                ...
                for trade in top_trades:
                    ...
                    if score >= threshold:
                        print(f"✅ BUY Triggered: {sym} | Score: {score}")
                        ...
                        print(f"🛑 REJECTED: {sym} | Score: {score} < {threshold}")
                
                # Small sleep to respect rate limits
                time.sleep(2)

    print("\n\n======================================")
    print("📊 FINAL DEBATE ENGINE P&L REPORT FOR LAST WEEK")
    if len(trades_executed) == 0:
        print("0 Trades executed after Debate Engine vetoes. Every single trade was rejected.")
    else:
        total_pnl = 0.0
        for t in trades_executed:
            print(f"[{t['Date']} {t['Time (IST)']}] {t['Symbol']} | P&L: {t['P&L (%)']}% | Score: {t['Score']}")
            total_pnl += t['P&L (%)']
        print(f"\nNet Cumulative P&L: {round(total_pnl, 2)}% | Total Trades: {len(trades_executed)}")
